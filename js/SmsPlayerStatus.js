...
let totalRewards = 0;
let totalDec = 0;
let totalLegendary,totalGold=0;
...
async function display(player, cards) {
	return new Promise(async function (resolve, reject) {
		...
		let balances = await get_balances(playe
let totalRewards = 0;
let totalDec = 0;
...
$(document).ready(async function () {
	...
	$('#view').submit(async function (e) {
		...
		htmlString += `</table>`;
		let summary = `<B>Total Season Loot Chests:</B>${totalRewards}<br/><B>Total DEC:</B>${totalDec.toFixed(3)}<br/><B>Total Legendary Potion:</B>${totalLegendary}<br/><B>Total Gold Potion:</B>${totalGold}`;
		$('div#summary').html(summary);
		$('div#display').html(htmlString);
